...
1. 打开本仓库根目录的 [`Bark空回／截断通知.json`](Bark空回／截断通知.json)，下载或复制内容。
...
```
st-bark-notify/
├── Bark空回／截断通知.json          ← 给酒馆「导入」用（推荐下载这个）
...
│   └── Bark空回／截断通知.json     与根目录导入 JSON 内容相同
...
| **根目录 `Bark空回／截断通知.json`** | 酒馆助手脚本导入格式 | `npm run gen:import` 可自动同步 CDN 地址 |
| **`dist/…/Bark空回／截断通知.json`** | 同上，放在 `dist` 旁备用 | 与根目录文件由同一脚本生成，内容一致 |
...
4. 提交 `src`、`dist`、`Bark空回／截断通知.json` 并 push 到 GitHub
